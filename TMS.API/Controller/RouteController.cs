...
namespace TMS.API.Controllers
{
    public class RouteController : TMSController<TMS.API.Models.Route>
    {
        ...
        protected override IQueryable<TMS.API.Models.Route> GetQuery()
        {
            var sql = string.Empty;
            sql += @$"
                    select *
                    from [{typeof(TMS.API.Models.Route).Name}]
                    where 1 = 1";
        ...
        public async Task<List<TMS.API.Models.Route>> ImportRoute([FromServices] IWebHostEnvironment host, List<IFormFile> fileImport)
        {
        ...
        public virtual async Task<OdataResult<TMS.API.Models.Route>> UserClick(ODataQueryOptions<TMS.API.Models.Route> options)
        {
            var sql = $@"select UserClick.Click,Route.Id,Route.ParentId,Route.Code,Route.Name,Route.Used,Route.Active,Route.InsertedBy,Route.InsertedDate,Route.UpdatedDate,Route.UpdatedBy
                                                from [{typeof(TMS.API.Models.Route).Name}]
                                                left join UserClick
                                                on [{typeof(TMS.API.Models.Route).Name}].Id = UserClick.RecordId
