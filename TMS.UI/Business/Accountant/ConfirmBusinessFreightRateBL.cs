using Bridge.Html5;
using Core.Clients;
using Core.Components;
using Core.Components.Extensions;
using Core.Components.Forms;
using Core.Extensions;
using Core.MVVM;
using Core.ViewModels;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using TMS.API.Models;
using TMS.API.ViewModels;

namespace TMS.UI.Business.Accountant
{
    public class ConfirmBusinessFreightRateBL : PopupEditor
    {
        public FreightRate freightRateEntity => Entity as FreightRate;
        public ConfirmBusinessFreightRateBL() : base(nameof(FreightRate))
        {
            Name = "Confirm Business Freight Rate";
        }

        public override void Cancel()
        {
            this.Dispose();
            base.Cancel();
        }

        public override void CancelWithoutAsk()
        {
            this.Dispose();
            base.CancelWithoutAsk();
        }

        public async Task SetInfo()
        {
            var vendor = await new Client(nameof(Vendor)).FirstOrDefaultAsync<Vendor>($"?$filter=Active eq true and TypeId eq 7551 and Id eq {freightRateEntity.BossId}");
            freightRateEntity.UserId = vendor is null ? null : vendor.UserId;
            UpdateView(false, nameof(FreightRate.UserId));
        }

        public void CreateBusinessFreightRate()
        {
            var confirm = new ConfirmDialog
            {
                Content = "Bạn có chắc chắn muốn lấy giá tham khảo sang biểu giá CVC không ?",
            };
            confirm.Render();
            confirm.YesConfirmed += async () =>
            {
                freightRateEntity.TypeId = 25156;
                await Save(freightRateEntity);
                Parent.Parent.UpdateView(true);
                Parent.Dispose();
                this.Dispose();
            };
        }
    }
}
