...
{
    public class FreightTransportationListBL : TabEditor
    {
        public Ledger selectedLedger { get; set; }
        public FreightTransportationListBL() : base(nameof(Ledger))
        {
        ...
        public async Task AddFreightTransportation()
        {
            await this.OpenPopup(
                featureName: "Freight Transportation Editor",
                factory: () =>
                {
                    var type = Type.GetType("TMS.UI.Business.Accountant.FreightTransportationEditBL");
        ...
        public async Task EditFreightTransportation(Ledger ledger)
        {
            await this.OpenPopup(
                featureName: "Freight Transportation Editor",
                factory: () =>
        ...
        {
            var ledgers = await new Client(nameof(Ledger)).GetRawList<Ledger>($"?$filter=Active eq true and InvoiceFormId eq 16047 and ParentId eq null and Id ge {selectedLedger} and TypeId eq null");
            var no = int.Parse(ledgers.FirstOrDefault().InvoiceNo.Substring(10));
            ledgers.ForEach(x =>
            {
                x.InvoiceNo = no < 10000 ? "CVC" + x.InvoiceDate.Value.ToString("yy") + "/" + x.InvoiceDate.Value.ToString("MM") + "-" + $"{no:0000}" : no.ToString();
                no++;
            });
        }
    }
