...
namespace Core.Extensions
{
    public static partial class Utils
    {
        ...
        public const string SqlReader = "SqlReader";
        ...
        {
            var type = entityType.GetComplexPropType(fieldName, obj);
            ...
            for (int i = 0; i < format.Length; i++)
            {
                ...
                switch (ch)
                {
                    case '{':
                        isInGroup = true;
                        beforeColon = true;
                        formatted.Append(ch + index.ToString());
                        break;
                    case ':':
        ...
        private static string GetCellTextInternal(Component header, object cellData, object row, Dictionary<string, List<object>> refData, bool server, bool emptyRow = false)
        {
            ...
            if (header.FieldName == IdField && cellData != null && cellData is int cellInt && cellInt <= 0)
            {
                return string.Empty;
            }
            if ((!isRef || !server) && header.FormatEntity.HasAnyChar())
            {
                ...
                {
                    return fn.Call(row, row).ToString();
                }
                return GetFormattedRow(header.FormatEntity, row);
            }
            else if (cellData == null)
            {
                return header.PlainText ?? string.Empty;
            }
            else if (cellData is DateTime || isDate)
            {
                return string.Format(header.FormatData ?? "{0:dd/MM/yyyy}", isDate ? dt : cellData as DateTime?);
            }
            else if (isRef)
            {
            ...
            {
                return string.Format(header.FormatData, cellData);
            }
            else
            {
        ...
        private static List<object> TryParseHeader(Component header)
        {
            try
            {
                return JsonConvert.DeserializeObject<List<Entity>>(header.Query).Cast<object>().ToList();
            }
            catch (Exception)
            {
                return null;
        ...
        {
            var parsed = bool.TryParse(value, out var res);
            if (parsed)
            {
                return res;
            }
        ...
        public static decimal? TryParseDecimal(this string value)
        {
            ...
            var parsed = decimal.TryParse(value, out var res);
            if (parsed)
            {
                return res;
            }
        ...
        public static object ChangeType(this string value, Type type)
        {
            ...
            {
                var parsed = int.TryParse(value, out var res);
                return parsed ? res : (type == typeof(int) ? 0 : (int?)null);
            }
            else if (type == typeof(long) || type == typeof(long?))
            {
                var parsed = long.TryParse(value, out var res);
                return parsed ? res : (type == typeof(long) ? 0 : (long?)null);
            }
            else if (type == typeof(short) || type == typeof(short?))
            {
                var parsed = short.TryParse(value, out var res);
                return parsed ? res : (type == typeof(short) ? 0 : (short?)null);
            }
            else if (type == typeof(bool) || type == typeof(bool?))
            {
            ...
            {
                var parsed = TimeSpan.TryParse(value, out TimeSpan res);
                return parsed ? res : (TimeSpan?)null;
            }
            else if (type == typeof(DateTime) || type == typeof(DateTime?))
            {
                var parsed = DateTime.TryParse(value, out DateTime res);
                return parsed ? res : (DateTime?)null;
            }
            else if (type == typeof(DateTimeOffset) || type == typeof(DateTimeOffset?))
            {
                var parsed = DateTimeOffset.TryParse(value, out DateTimeOffset res);
                return parsed ? res : (DateTimeOffset?)null;
            }
            else if (type == typeof(byte) || type == typeof(byte?))
            {
            ...
            {
                var parsed = decimal.TryParse(value, out var res);
                return parsed ? res : (type == typeof(decimal) ? 0 : (decimal?)null);
            }
            else if (type == typeof(double) || type == typeof(double?))
            {
                var parsed = float.TryParse(value, out var res);
                return parsed ? res : (type == typeof(double) ? 0 : (double?)null);
            }
            else if (type == typeof(float) || type == typeof(float?))
            {
                var parsed = float.TryParse(value, out var res);
                return parsed ? res : (type == typeof(float) ? 0 : (float?)null);
            }
            return null;
        }
